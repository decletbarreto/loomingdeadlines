...
library(parallel)
library(foreach)
# .county.subdivision.geoid = crisfield.cs.fips[1]
# .inundation.scenario      = east.scenario
# .infrastructure.sf        = crisfield.ci.sf
...
cores <- 12
crisfield.cs.fips <- c("2403990628", "2403990720", "2403991084")
...
scenario.counter <- 1
#east.inundation.scenarios <- east.inundation.scenarios.list[28]
crisfield.ci.sf = read_sf("/home/rama/coastal_deadline/data/Crisfield/crisfield_assets_04192023.shp")

for(east.scenario in east.inundation.scenarios.list)
{
  # Initialize objects to store results
  crisfield.inundation.summary             <- list()
  crisfield.inundated.named.infrastructure <- st_sf(data.frame(), geometry = st_sfc(),crs = (assign.crs("EAST"))) #empty sf with correct crs
  crisfield.npl.inundated                  <- st_sf(data.frame(), geometry = st_sfc(),crs = (assign.crs("EAST"))) #empty sf with correct crs

  crisfield.inundation.layer.summary.logfile <- file.path(paste(log.dir, "/", east.scenario, "_crisfield_inundation_layer_summary.log",sep=""))
  unlink(crisfield.inundation.layer.summary.logfile)
  # Loop in parallel through county subdivisions
  # Register parallel backend
  cl <- makeCluster(cores, outfile = crisfield.inundation.layer.summary.logfile)
  registerDoParallel(cl)
  results <- foreach(cs.fips = crisfield.cs.fips,
  ...
  # Combine the results
  for (res in results) {
    crisfield.inundation.summary             <- rbind(crisfield.inundation.summary,             res$inundation.summary)
    crisfield.inundated.named.infrastructure <- rbind(crisfield.inundated.named.infrastructure, res$inundated.named.infrastructure)
    crisfield.npl.inundated                  <- rbind(crisfield.npl.inundated,                  res$npl.inundated)
  }
  ...
  cs.crisfield.inundated.named.infrastructure.layer.name <- paste(output.dir, "/crisfield/", east.scenario, "_inundated_named_infrastructure.gpkg", sep="")
  print(paste("writing inundated named infrastructure to GPKG: ", cs.crisfield.inundated.named.infrastructure.layer.name, sep=""))
  write_sf(obj = crisfield.inundated.named.infrastructure, dsn = cs.crisfield.inundated.named.infrastructure.layer.name, driver = "GPKG", delete_layer = TRUE)
  ...
  print(paste("FINISHED ", east.scenario, sep=""))
  #increase the scenario counter
  scenario.counter <- scenario.counter + 1
}

#Assemble AGOL inundated CI data
crisfield.ci.df <-  read_sf("/home/rama/coastal_deadline/data/Crisfield/crisfield_assets_04192023.shp") %>%
          st_drop_geometry()

inundated.list <- list.files(paste(output.dir, "/crisfield",sep=""), pattern=".gpkg$", full.names = TRUE)
#all.inundated.sf <- read_sf(inundated.list[1]) %>% st_transform(., 4326)
all.inundated.sf <- st_sf(data.frame(), geometry = st_sfc(),crs = st_crs(4326)) %>% select(-starts_with("is_flooded")) #empty sf with correct crs
...
all.inundated.type.wide <- all.inundated.df %>%
  ...
  st_drop_geometry()                 %>%
  pivot_wider(., id_cols = Name, names_from = ysf, values_from = row.number, values_fn = list(row.number = length)) %>%
  ...
  inner_join(x  = .,  #join name, address, city, state, zip, lat, lon
             ...
             multiple = "first") %>%
  inner_join(x  = .,
             y  = crisfield.ci.df[,"Name"],
             by = "Name",
             multiple = "first") %>%
  st_as_sf(., coords = c("lon", "lat"), crs = 4326) %>%
  select(Name, Group, GROUPID, Descriptio, everything()) %>%
  select(-c(provenance,year, scenario, inundation.frequency))  %>%
  write_sf(., dsn = paste(output.dir, "/crisfield/crisfield_inundated_critical_infrastructure", sep=""), driver="ESRI Shapefile", delete_layer = TRUE)

shp.zip <- paste(output.dir, "/crisfield/crisfield_inundated_critical_infrastructure.zip",sep="")
shps <- list.files(path = paste(output.dir, "/crisfield/crisfield_inundated_critical_infrastructure",sep=""), pattern= "inundated_critical_infrastructure*", full.names = T)
zip(shp.zip,shps)
